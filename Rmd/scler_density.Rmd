...
ggplot(df2, aes(x = dist, y = dens, color = timepoint)) +
  facet_grid(dir ~ reef) +
  geom_point()
```
...
```{r}
r2s0 <- filter(df2, reef == "R2", dir == "S", timepoint == "post-2014")
r2s1 <- filter(r2s0, date != "2017-03-29")

# Fit a linear model with all points and calculate Cook's distance (cannot calculate Cook's for poisson glm)
mod2 <- lm(n/transect_area_m2 ~ dist, data = r2s0) 
nd2 <- crossing(dist = r2s0$dist, transect_area_m2 = 1)
nd2$fit <- predict(mod2, newdata = nd2, re.form = NA, type = "response")
# Plot fitted lm
ggplot(r2s0, aes(x = dist, y = dens, color = timepoint)) +
  geom_point() +
  geom_line(data = nd2, aes(x = dist, y = fit), inherit.aes = FALSE)
# Plot cook's distance ~ distance
plot(cooks.distance(mod2) ~ r2s0$dist)

## Get cook's distance for this point
max(cooks.distance(mod2))
## Get second highest cook's distance
rev(sort(cooks.distance(mod2)))[2]
```

### Fit poisson regression on count data
```{r}
# Remove outlier point from R2S
df2 <- filter(df2, date != "2017-03-29")
...
# Save as RData
save(newdata2, anno, file = "data/dens.mod.RData")
...
# Change in density of small corals (1-2cm) (2009 vs. 2016-2017)
Transects at increasing distances from channel were recorded in 2009 baseline surveys and 2016/2017 cross-site surveys, when small corals were recorded.

```{r}
# Subset 2009 baseline and 2016/2017 cross-site survey data <1000 m from channel
df3 <- scler %>%                # Select dataset that includes all corals, including small
...
```{r}
# Fit model
mod3 <- glmer(n ~ dist * reef * dir * timepoint +      # count as response
             offset(log(transect_area_m2)) +          # offset accounts for diff transect sizes
             (1|site/site2/transect) + (1|date),            # random effects
             data = df3, family = "poisson")           # poisson model

# Get fitted responses
newdata3 <- df3 %>%
  tidyr::expand(nesting(reef, dir, timepoint, dist), 
                transect_area_m2 = 1)        # transect area of 1 m2 will predict count per m2
newdata3$fit <- predict(mod3, newdata3, type = "response", re.form = NA)

# Get confidence intervals by simulation
bootfit3 <- bootMer(mod3, FUN = function(x) predict(x, newdata3, type = "response", re.form = NA), 
                   nsim = 20, parallel = "multicore", ncpus = 4)
newdata3$lci <- apply(bootfit3$t, 2, quantile, 0.08)
newdata3$uci <- apply(bootfit3$t, 2, quantile, 0.92)
```

### Plot fits and test for change in density near channel
```{r}
# Plot raw data and fitted results
ggplot(newdata3, aes(x = dist, y = fit, color = timepoint)) +
  facet_grid(dir ~ reef) +
  geom_ribbon(aes(ymin = lci, ymax = uci, linetype = NA), alpha = 0.2) +
  geom_line() +
  geom_point(data = df3, aes(y = dens)) +
  labs(x = "Distance from channel", y = "# corals per m2")

# Get fitted value of coral density adjacent to channel (i.e., dist = 20 m)
lsm20 <- lsmeans(mod3, specs = c("reef", "dir", "timepoint"),
                 at = list(dist = 20, transect_area_m2 = 1), transform = "response")

# Get fitted value of coral density away from channel (i.e., dist = 300 m)
lsm300 <- lsmeans(mod3, specs = c("reef", "dir", "timepoint"), 
                  at = list(dist = 300, transect_area_m2 = 1), transform = "response")
...
#anno <- full_join(fits, sigs)
anno <- full_join(fits, sigs) %>%
  mutate(rel = -(estimate) / `2009 baseline` * 100,
         rel2 = ifelse(p.value > 0.1, "", paste0(round(rel, 1), "%")),
         sig = gtools::stars.pval(p.value),
         sig2 = gsub(" ", "n.d.", sig))

save(newdata3, anno, file = "data/small.dens.mod.RData")

# Plot fitted results
ggplot(data = newdata3, aes(x = dist, y = fit, color = timepoint)) +
  facet_grid(dir ~ reef, scales = "free_x") +
  geom_line() +
  scale_x_continuous(limits = c(0, 500)) +
  geom_ribbon(aes(ymin = lci, ymax = uci, linetype = NA), alpha = 0.2) +
