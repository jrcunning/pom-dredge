...
bootfit_1 <- bootMer(mod_1, FUN = function(x) predict(x, newdata_1, type = "response", re.form = NA), 
                     nsim = 100, parallel = "multicore", ncpus = 4)
...
newdata_2 <- filter(newdata_2, reef == "R2" | (reef == "R3" & dir == "N") | (reef == "R3" & dir == "S" & timepoint == "2009 baseline"))
...
anno <- anno %>%
  mutate(`2009 baseline` = ifelse((reef == "HB" | (reef == "R3" & dir == "S")), NA, `2009 baseline`),
