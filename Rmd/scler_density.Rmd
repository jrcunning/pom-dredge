...
newdata_1$lci <- apply(bootfit_1$t, 2, quantile, 0.05)
newdata_1$uci <- apply(bootfit_1$t, 2, quantile, 0.95) 
...
bootfit_2 <- bootMer(mod_3, FUN = function(x) predict(x, newdata_2, type = "response", re.form = NA), 
                     nsim = 400, parallel = "multicore", ncpus = 8)
newdata_2$lci <- apply(bootfit_2$t, 2, quantile, 0.05)
newdata_2$uci <- apply(bootfit_2$t, 2, quantile, 0.95)
```
