...
# ## Different sites were observed on different dates. For this reason, we will bin dates by week number 
# ## starting at the earliest date on which data were collected.
# alldata <- alldata %>%
...
Show the cumulative proportion of corals experiencing partial mortality at each site over time.
...
Calculate the total number of corals at each site that experienced partial mortality (up to July 2015), and plot as a barplot.
...
#### ERRORS HAVE CHANGED AND ARE MUCH SMALLER... DUE TO UPDATE TO LSMEANS PACKAGE?
...
**We find a significantly higher probability of partial mortality due to sedimentation for corals at channelside sites relative to controls for the northern middle reef (76% vs. 3%), the southern middle reef (61% vs. 15%), and the southern hardbottom (70% vs. 22%).**
There was an almost significant difference in probability of partial mortality at the southern outer reef channelside sites (23%) vs. control sites (7%; p = 0.06).
...
**When all reef locations are analyzed together, partial mortality due to sediment occurred in 52.8% of corals at channelside sites compared to only 10.5% at control sites (p = 0.0001).**
...
**Corals on the northern middle reef (R2 N) showed increased tissue loss at channelside sites (49.3%) relative to control sites (15.2%).**
...
For *Stephanocoenia intersepta*, tissue loss is also higher at channelside relative to control sites (57.0% vs. 8.3%), but this difference was not significant. For *P. astreoides*, there were no tagged corals at the channelside sites at the northern middle reef.


# Look at SSID and PAST at other sites or across all sites?
...
coarse <- ggplot(coarsesed.pm, aes(x = int_fit, y = prob)) + 
  ...
  #geom_smooth(method='lm') +
  labs(x = "log Total coarse sediment deposited (g)",
       y = "Probability of partial mortality to coral",
