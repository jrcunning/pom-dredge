...
```{r}
cumpm <- allweeks %>%
  mutate(PM = hasCond("^PM$", Condition.Codes)) %>%             # Did coral have PM recorded?
  group_by(coral) %>%
  mutate(PM = ifelse(any(PM) & weekn > min(weekn[PM]), TRUE, PM)) %>%
  group_by(reef, dir, Channel, Site, weekn) %>%
  summarise(prop = sum(PM) / n())

ggplot(cumpm, aes(x = weekn, y = prop, color = Channel)) + 
  facet_wrap(~ Site) +
  geom_line() +
  labs(title = "Cumulative PM", x = "Time (weeks)", y = "Proportion of corals")

ggplot(cumpm, aes(x = weekn, y = prop, color = Channel, group = Site)) + 
  geom_line() +
  labs(title = "Cumulative PM", x = "Time (weeks)", y = "Proportion of corals")
```
