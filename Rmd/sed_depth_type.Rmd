...
# Load tidy sediment depth and type data
```{r}
load(file = "data/substrate.RData")
```
...
save(LR, resdf, file = "data/sdepthmod.RData")
...
# Sediment depth at permanent control sites?
```{r}
substrate %>% filter(perm == "yes", channel == "control") %>%
  group_by(reef, dir) %>%
  summarise(median = median(sdepth_cm))

substrate %>% filter(perm == "yes", channel == "control") %>%
  ggplot(aes(x = reef:dir, y = sdepth_cm)) +
  geom_boxplot()

# Percent of measurements greater than 1 cm
substrate %>% filter(perm == "yes", channel == "control") %>%
  group_by(reef, dir) %>%
  summarise(sum(sdepth_cm > 1) / n())
```
