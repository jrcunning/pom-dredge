...
# Percent of measurements > 0.5 cm
```{r}
over_0.5 <- LR %>%
  group_by(reef, dir, dist) %>%
  summarise(N = n(),
            n_over_0.5 = sum(sdepth_cm > 0.5),
            pct_over_0.5 = n_over_0.5 / N)
...
over_5 <- LR %>%
  group_by(reef, dir, dist) %>%
  summarise(N = n(),
            n_over_5 = sum(sdepth_cm > 5),
            pct_over_5 = n_over_5 / N)
