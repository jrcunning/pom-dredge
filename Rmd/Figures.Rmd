...
theme_custom <- function() {
  theme_bw(base_size = 8, base_family = "Arial") %+replace%
    theme(
...
```{r FIGS1, echo = FALSE}
fine.gamms.reef.dir.fits <- fine.gamms.reef.dir.fits %>%
  mutate(dist_cat = factor(case_when(
    channel == "channelside" ~ "< 50 m",
    reef == "HB" & channel == "control" ~ "1.25 - 2.5 km",
    reef %in% c("R2", "R3") & channel == "control" & dir == "S" ~ "1.25 - 2.5 km",
...
ggplot(filter(sed, var == "cov"), aes(x = date, y = fit, color = dist_cat)) +
  ...
  geom_vline(xintercept = c(dredge.start.date, dredge.end.date), linetype = 3, lwd = 0.25) +
  labs(title = "Figure 1.",
       x = NULL, y = "Percent sand cover",
       color = "Distance to channel") +
...
ggplot(data = droplevels(sedtrapmod), mapping = aes(x = date, y = fit, color = dist_cat)) + 
  facet_grid(dir ~ reef + dist_cat, scales = "free", drop = TRUE) +
...
ggplot(fine.depo, aes(x = dist_cat, y = int_fit/1000, fill = dist_cat)) +
  ...
  labs(title = "Figure 2.",
       y = "Grams / cm2", x = "Distance from channel") +
...
ggplot(gamms2.fitted, aes(x = date, y = fit, color = dist_cat)) +
  ...
  geom_vline(xintercept = c(dredge.start.date, dredge.end.date), linetype = 3, lwd = 0.25) +
  scale_color_manual(values = c("#fc8d62", "#8da0cb", "#66c2a5")) +
  labs(title = "Figure 3.", x = NULL, y = "Probability of partial or complete burial",
       color = "Distance from channel") +
...
ggplot(pm.lsm1, aes(x = dist_cat, y = prob, fill = dist_cat)) + 
  ...
  theme(legend.position = "none",
        axis.text.x = element_text(angle=0, hjust=0.5, vjust = 1)) +
  labs(title = "Figure S3.", x = "Distance from channel", y = "Probability of partial mortality due to sediment")
...
ggplot(finesed.pm, aes(x = int_fit/100, y = prob)) + 
  ...
  labs(title = "Figure 4.", x = "Total fine sediment deposited (kg / m2)",
       y = "Probability of partial mortality", color = "Distance from channel") +
  geom_text_repel(aes(label = paste(reef, dir)), size = 2.5) +
  scale_color_manual(values = c("#fc8d62", "#8da0cb", "#66c2a5")) +
...
ggplot(dead.lsm4, aes(x = dist_cat, y = prob, fill = dist_cat)) + 
  ...
  labs(title = "Figure S4.", 
       x = "Distance from channel", y = "Probability of total mortality (non-disease)")
...
ggplot(data = large.dens, aes(x = dist, y = fit, color = timepoint)) +
  ...
  labs(title = "Figure 5.", x = "Distance from channel (m)", y = "# corals > 3cm per m2",
       color = "") +
...
ggplot(data = small.dens, aes(x = dist, y = fit, color = timepoint)) +
  ...
  labs(title = "Figure S5.", x = "Distance from channel (m)", y = "# small corals (1-2 cm) per m2", color = "") +
...
ggplot(df, aes(x = diameter, fill = year <= 2009)) + 
  ...
  facet_grid(dir ~ reef) +
  labs(title = "Figure 6.",
       x = "Coral diameter (cm)", y = "Proportion of individuals", fill = "") +
  xlim(0, 20) +
...
ba %>%
  ...
  theme_custom() +
  labs(title = "Figure S6.", x = "", y = "Percent sand cover") +
  scale_y_continuous(labels = scales::percent) +
  theme(legend.position = c(0.2, 0.8))

ggsave(filename = "figures/FigS6.png", 
       device = "png", width = 5, height = 3, units = "in")
```
