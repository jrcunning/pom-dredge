...
```{r Fig7, fig.height = 5, fig.width = 5}
...
```{r Fig8, fig.height = 3.74016, fig.width = 7.4801}
load("data/large.no2015.RData"); large.dens <- newdata_1; large.anno <- anno; large.lsm <- lsm; rm(anno, newdata_1, lsm)
...
**Figure 8. Density of corals ≥3cm within 500m from the channel in each monitoring region before and after dredging operations.** Data 'before' dredging were collected in 2010 and 2013, and data after dredging were collected in 2015-2017. GLMM fits are shown as lines for the regions and times in which data were collected at >2 points beyond 50m, and otherwise as points including only data from within 50m of the channel. Shaded regions and error bars represent 84% CI's. Text annotations indicate the results of tests for differences between timepoints at 20m and 300m from the channel (*** = p < 0.0001; ns = not significantly different).

\  

```{r FIGS6, fig.height = 3.74016, fig.width = 7.4801}
load("data/small.no2015.RData"); small.dens <- newdata_2; small.anno <- anno; rm(anno, newdata_2)
...
```{r Fig9_alternative, fig.height = 3.74016, fig.width = 7.4801}
...
**Figure 9 - alternative.** *Alternative version of Figure 8, with absolute density instead of relative proportion on y-axis.*
...
#Sediment type - not useful enough?
df <- substrate %>%
  filter(!is.na(stype)) %>%
...
ggsave(filename = "figures/FigS7.png", device = "png", width = 90, height = 65, units = "mm")
```
...
```{r Fig11, fig.height = 7.4801, fig.height = 7.4801}
# Plume presence
load("data/plume.RData")
plume <- plume.summ %>%
...
Fig11 <- df %>% gatherpairs(plumeprob, int_finedepo, mean_sandcov, mean_sedstress, pmort) %>%
  filter(.xkey != .ykey) %>%
