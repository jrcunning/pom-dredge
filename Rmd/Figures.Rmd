...
### *Sediment plume presence based on satellite data*
...
### Plot fitted models
ggplot(gamm.fits, aes(x = date, y = fit, color = dist_cat)) +
  ...
  labs(x = "Date", y = "Probability of dredge plume presence", color = "Distance from channel") +
...
### Plot fitted models
fig2 <- ggplot(fine.gamms.reef.dir.fits, aes(x = middate, y = fit, color = dist_cat)) +
  facet_grid(dir ~ reef, labeller = global_labeller) +
...
fig3 <- ggplot(filter(sed, var == "cov"), aes(x = date, y = fit, color = dist_cat)) +
  facet_grid(dir ~ reef, labeller = global_labeller) +
...
ggsave(filename = "figures/Fig3.png", plot = fig3, device = "png", width = 190, height = 95, units = "mm")
...
figS1 <- fig3 + scale_x_date(limits = NULL)
...
# Plot
fig5 <- ggplot(gamms2.fitted, aes(x = date, y = fit, color = dist_cat)) +
  facet_grid(dir ~ reef, labeller = global_labeller) +
...
figS3 <- fig5 + scale_x_date(limits = NULL)
...
```{r FIG6, fig.height = 5, fig.width = 5}
...
**Figure 7. Density of corals ≥3cm within 500m from the channel in each monitoring region before and after dredging operations.** Data 'before' dredging were collected in 2010 and 2013, and data after dredging were collected in 2015-2017. GLMM fits are shown as lines for the regions and times in which data were collected at >2 points beyond 50m, and otherwise as points including only data from within 50m of the channel. Shaded regions and error bars represent 84% CI's. Text annotations indicate the results of tests for differences between timepoints at 20m and 300m from the channel (*** = p < 0.0001; ns = not significantly different).
...
```{r FIG8_alternative, fig.height = 3.74016, fig.width = 7.4801}
...
sandcov <- gamms.reef.dir.fits %>%
  ...
  summarise(int_sandcov = sum(fit)) %>%
  select(reef, dir, channel, int_sandcov)
...
stress <- gamms2.fitted %>%
  group_by(reef, dir, channel) %>%
  filter(date > dredge.start.date, date < dredge.end.date) %>%
  summarise(int_sedstress = sum(fit)) %>%
  select(reef, dir, channel, int_sedstress)
...
keynames <- c(plumeprob = "Dredge plume",
              int_sedstress = "Coral burial",
              int_sandcov = "Benthic sed.",
...
df %>% gatherpairs(plumeprob, int_finedepo, int_sandcov, int_sedstress, pmort) %>%
  filter(.xkey != .ykey) %>%
  mutate(.xkey = factor(.xkey, levels = c("plumeprob", "int_finedepo", "int_sandcov", "int_sedstress", "pmort")),
