---
title: "Manuscript Figures and Supplementary Figures"
output:
  html_document:
    toc: true
    toc_float:
...
plume_points <- filter(plume, reef %in% c("HB", "R2", "R3"), dir %in% c("N", "S")) %>%
  mutate(yval = case_when(dist_cat == "< 50 m" ~ as.numeric(plume) + (0.015 * sign(as.numeric(plume) - 0.1)),
                          dist_cat == "1.25 - 2.5 km" ~ as.numeric(plume) + (0.03 * sign(as.numeric(plume) - 0.1)),
                          dist_cat == "9.38 km" ~ as.numeric(plume) + (0.03 * sign(as.numeric(plume) - 0.1))))
...
ggplot(gamm.fits, aes(x = date, y = fit, color = dist_cat)) +
  ...
  geom_point(data = plume_points, mapping = aes(y = yval, color = dist_cat), pch = 20, alpha = 0.5, cex = 0.5) +
  scale_color_manual(values = c("#fc8d62", "#8da0cb", "#66c2a5")) +
  scale_y_continuous(limits = c(-0.04, 1.04), expand = expand_scale(add = 0.01)) +
  geom_hline(yintercept = c(0, 1), lwd = 0.25) +
  theme_custom() +
...
### Plot fitted models
Fig4 <- ggplot(fine.gamms.reef.dir.fits, aes(x = middate, y = fit, color = dist_cat)) +
  facet_grid(dir ~ reef, labeller = global_labeller) +
  ...
  labs(x = NULL, y = "Fine sediment trap accumulation rate (mg/cm2/d)",
...
Fig6 <- ggplot(filter(sed, var == "cov"), aes(x = date, y = fit, color = dist_cat)) +
  facet_grid(dir ~ reef, labeller = global_labeller) +
...
figS1 <- Fig6 + scale_x_date(limits = NULL)
...
ggplot(data = droplevels(sedtrapmod), mapping = aes(x = date, y = fit, color = dist_cat)) + 
  ...
  scale_y_continuous("Fine sediment accumulation rate (mg/cm2/day)",
...
# Plot
Fig7 <- ggplot(gamms2.fitted, aes(x = date, y = fit, color = dist_cat)) +
  facet_grid(dir ~ reef, labeller = global_labeller) +
...
ggsave(filename = "figures/Fig7.png", plot = Fig7, device = "png", width = 190, height = 95, units = "mm")
...
figS3 <- Fig7 + scale_x_date(limits = NULL)
...
```{r old_Fig8, include = FALSE, eval = FALSE, fig.height = 5, fig.width = 5}
...
ggsave(filename = "figures/Fig8.png", device = "png", width = 90, height = 85, units = "mm")

#**Figure 8. Relationship between total fine sediment deposition and coral partial mortality during dredging operations.** Points indicate, for each #monitoring area, the total amount of fine sediments deposited during dredging (±95% CI; see Fig. 4) on the x-axis, and the probability of partial #mortality (±95% CI; see Fig. S3) on the y-axis. Points are labeled corresponding to reef region (SIR = sourthern inner reef, NIR = northern inner reef, #SMR = southern middle reef, NMR = northern middle reef, SOR = southern outer reef, NOR = northern outer reef) and colored corresponding to distance from #channel. The fitted dose-response curve follows a four parameter type two Weibull function weighted inversely by the SE of partial mortality estimates.
...
```{r Fig9, fig.height = 7.4801, fig.height = 7.4801}
# Plume presence
load("data/processed/plume.RData")
plume <- plume.summ %>%
...
Fig9 <- df %>% gatherpairs(plumeprob, int_finedepo, mean_sandcov, mean_sedstress, pmort) %>%
  filter(.xkey != .ykey) %>%
...
ggsave(filename = "figures/Fig9.png", device = "png", width = 190, height = 190, units = "mm")
...
```{r FIG13, fig.height = 3.74016, fig.width = 7.4801}
...
ggsave(filename = "figures/Fig13.png", device = "png", width = 190, height = 95, units = "mm")
```
