...
ggplot(filter(sed, var == "cov"), aes(x = date, y = fit, color = channel)) +
  facet_grid(dir ~ reef) +
...
fig3 <- ggplot(summary(pm.lsm1), aes(x = channel, y = prob, fill = channel)) + 
  facet_grid(dir ~ reef) +
...
```{r}
load("data/substrate.RData")

ggplot(data = filter(substrate, dist < 1000, reef2 == "LR"), 
