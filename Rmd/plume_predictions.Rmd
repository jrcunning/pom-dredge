...
fit.plumeprob <- gamm.fits %>%
  group_by(reef, siteno, site) %>%
  summarise(plumeprob = mean(fit))
...
par(mar = c(2.5, 2.6, 1, 11), mgp = c(1.25, -0.1, 0), tcl = 0.3, cex.axis = 0.8)
...
axis(2, at = yticks)
axis(4, line = 0, at = yticks, labels = ascale(yticks, mod.finedepo, 0))
mtext(4, line = 1, text = "Predicted sediment deposition (kg/m2)")
axis(4, line = 4, at = yticks, labels = ascale(yticks, mod.sandcov, 2))
mtext(4, line = 5, text = "Predicted mean daily sediment cover")
axis(4, line = 8, at = yticks, labels = ascale(yticks, mod.pmort, 2))
mtext(4, line = 9, text = "Predicted probability of coral partial mortality")
lines(plumeprob ~ dist, data = subset(resdf, reef == "MR"), col = "red")
lines(plumeprob ~ dist, data = subset(resdf, reef == "OR"), col = "blue")
legend("topleft", legend = c("Nearshore Ridge (NR)", "Inner Reef (IR)", "Outer Reef (OR)"), 
