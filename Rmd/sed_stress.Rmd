...
dredge.start.day <- (dredge.start.date - min(tagged$date)) / ddays(1)
dredge.end.date <- as.Date("2014-12-31")                  # DCA report August 2015, page 3
dredge.end.day <- (dredge.end.date - min(tagged$date)) / ddays(1)
```
...
ggplot(sedstress.summ1, aes(x = day, y = prop, color = channel)) + 
  ...
  labs(title = "Sedimentation stress (SA, PBUR, or BUR)",
       x = "Time (days)", y = "Proportion of corals")
```
...
gamms1.fitted <- gamms1 %>% unnest(newdata, fit, lci, uci)
```

```{r}
# Plot
fig1 <- ggplot(gamms1.fitted, aes(x = day, y = fit, color = channel)) +
  facet_grid(dir ~ reef) +
  geom_point(data = sedstress.summ1, aes(y = prop, size = n), alpha = 0.4) +
  scale_size(range = c(0.1, 2.5)) +
  geom_ribbon(aes(ymin = lci, ymax = uci, linetype = NA), alpha = 0.2) + 
  geom_line(lwd = 1) +
  geom_vline(xintercept = c(dredge.start.day, dredge.end.day), linetype = 1, lwd = 0.25) +
  labs(title = "Sedimentation stress (SA, PBUR, or BUR)",
       x = "Time (days)", y = "Proportion of corals")
...
gamms2.fitted <- gamms2 %>% unnest(newdata, fit, lci, uci)
```

```{r}
# Plot
fig2 <- ggplot(gamms2.fitted, aes(x = day, y = fit, color = channel)) +
  facet_grid(dir ~ reef) +
