...
tagged_bool_f <- tagged_bool %>%
  ...
  mutate(dates = map(data, ~ sort(unique(.$date))),                           # Which weeks have data
          runs = map(dates, ~ split(.x, cumsum(c(TRUE, diff(.x) > 70)))), # Find runs w/o 140d gaps
...
save(sedstress.summ2, gamms2.fitted, file = "data/sedstress_gamms.RData")
```
