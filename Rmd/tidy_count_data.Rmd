...
dens2015 <- readxl::read_xlsx(
  ...
  select(date, site, transect, coral_id, category, subcategory, diameter, count) %>%
...
dens2016scler <- readxl::read_xlsx(
  ...
  mutate(count = 1,  # 1 coral per row (no count data in sheet)
         date = as.Date(date)) %>%
  distinct()   # reduce to single row for each site/transect/coral_id (b/c multiple conditions/rows)
