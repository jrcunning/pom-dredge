...
cs2016octo <- cs2016octo %>%
  ...
  mutate(site2 = str_extract(site, "LR|RR|CP|SG|CR")) %>%            # extract LR or RR from site name
  mutate(site = str_extract(site, "^.[^-|_]*")) %>%                 # extract base site form site name
...
all_counts <- all_counts %>%
  mutate(site2 = ifelse(perm == "yes", case_when(
    ...
    site %in% c("R2NC1", "R2NC3", "R2N1", "R2SC2", "R2S2", 
                ...
                "R2N2", "R3S2") ~ "LR",
    site %in% c("R3S1", "R3SC1", "HBNC1", "HBS1", "HBS2", "HBS3", "HBN3") ~ "CP",
