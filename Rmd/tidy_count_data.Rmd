...
dens2009 <- readxl::read_xlsx(
  ...
  mutate(category = str_replace_all(category, "Octocoral", "octocoral")) %>% # Fix capitalization error
  mutate(category = str_replace_all(category, "Scleractinian", "scleractinian")) %>%
...
all_counts <- data_frame(date = c("2009", "2013", "2016")) %>%
  mutate(data = list(dens2009, dens2013, dens2016)) %>%
  unnest(data)

# Subset octocorals
octo_counts <- all_counts %>%
  filter(category == "octocoral")


unique(all_counts$site)   # need to fix site names for consistency
```
