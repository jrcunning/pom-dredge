...
This document attempts to replicate results presented in Precht et al. (2016) in *Scientific Reports* [(doi:10.1038/srep31374)](http://doi.org/10.1038/srep31374) using the dataset provided to us by the authors upon request on March 8, 2018. After importing the data, we proceed line by line through the 'Tagged-coral colonies' section of the results, with block quotes taken directly from the paper, followed by our attempt to reproduce each result. For full transparency and reproducibility, this document contains the R code we used in each attempt to reproduce reported results, along with output in the form of figures and tables. Finally, we describe and explain the outcome of our attempts to reproduce each of the authors' reported findings.

In short, we are unable to reproduce many of the reported findings. Some discrepancies in the data are more dramatic than others. Through follow-up communications with the authors, we now understand that a major factor in our inability to reproduce the disease prevalence results is that the data were changed post hoc: additional corals found dead were assumed to have died from white plague disease (without having ever been observed with the disease), based on meeting a set of criteria that are not described in the Methods[^1]. This additional purported plague-related mortality was included in reported disease prevalence values in the paper, but these changes were not reflected in the initial dataset provided to us. An additional summary spreadsheet [(Copy of R2_Ctrls_PCWK4_Verify.xlsx)](Copy of R2_Ctrls_PCWK4_Verify.xlsx) indicating which additional corals were assumed to have died from disease was subsequently provided to us on April 4, 2018. Further obfuscating disease prevalence values, corals that had already died (potentially weeks to months earlier) also appear to be included in reported disease prevalence levels[^2]. The inclusion of dead corals in disease prevalence values is in contrast to the authors’ definition of disease prevalence in the Methods, which states, “We define white-plague prevalence as the proportion of cases that were present in the population, at a given time.” 

[^1]: In the Methods section, the authors state only, “In addition, in cases where corals had died, we were generally able to discern the exact cause of mortality by carefully evaluating the sequence of events recorded (and photographed), prior to death.” The criteria are not described, and it is not made clear that dead corals or corals assumed to have died from disease are also included in “disease prevalence” values.
...
[^6]: There is evidence to suggest that control locations from which they report data to the south of the channel were likely also impacted by dredging sediment (P. Swart, University of Miami, 28 June 2016, “Report on the Mineralogy and the Stable Carbon and Oxygen Isotopic Composition of Samples Supplied by NOAA”).

![Figure 1. Map of all 26 reef sites monitored during the dredging project, and the 4 sites included in the Precht et al. *Scientific Reports* paper.](map_figure.jpg)
...
Here we verify whether the species IDs associated with each tagged coral are the same across all observations.
...
Out of 115 tagged corals, 23 have multiple species IDs associated with them at different points in time, indicative of some (unidentifiable) error in data entry. A full list of these conflicting species assignments is provided in the attached file: [precht_spp_conflicts.txt](precht_spp_conflicts.txt).

#### 1.2 Change conflicting species assignments to reflect most frequently assigned species

We attempt to fix these conflicting species assignments in the present analysis by changing species assignments for each coral (defined as unique site-transect-coralID) to the most frequently assigned species. In response to our inquiry, authors also include a verification of these species IDs at [Copy of Conflicting Coral IDs verified.xlsx](Copy of Conflicting Coral IDs verified.xlsx).
...
Within the dataset provided, we confirm that 115 coral colonies of 13 different species were monitored. However, we found that individual colonies were monitored between 31 and 45 times each. This is inconsistent with the statement that colonies were observed at least 40 times.
...
Using the data provided by the authors, we find that the prevalence of white plague disease (“WP”) on Sept. 26, 2014 was not greater than 5% at southern monitoring sites. Disease prevalence was only 4% at site R2SC2 (N=1 out of 25 corals) and was 0% at R2SC1 (R2SC2 and R2SC1 are the southern monitoring sites referenced in the Results statement above). Neither reached the “5% prevalence” threshold on that date.
In response to our inquiry regarding this discrepancy, the authors responded that “the original statement should have read ‘< 5%’ not ‘> 5%.’” For Authors’ response to our inquiry about this issue, see [Addl Comments on Response to Rachel Silverstein - Part 2.pdf](Addl Comments on Response to Rachel Silverstein - Part 2.pdf). This apparent typographical error fundamentally changes the conclusions of the paper and the contagion-based model presented in the paper.
...
If we also include corals recorded with white plague that had then died (12.7%, 7 out of 55 colonies observed), we still do not find a 15% disease prevalence. Based on communication with the authors (see [Response to Sci Rep questions from RS.pdf](Response to Sci Rep questions from RS.pdf)), it appears that the remaining discrepancy of 2.3% is due to the authors' inclusion of additional corals that were dead that were never recorded as having WP. These were later assumed to be disease-related deaths and are not included in the initial dataset. (Authors stated that certain corals that were found dead were assumed to have died from white plague disease, based on a set of criteria that is not described in the paper). The definition of disease prevalence in the Methods section does not include dead corals. Therefore, the reported value does not accurately describe the colonies that “showed outward signs of white plague” on this date.
...
We also find that WP was first observed ≥5% at the northern sites in June 2015 within the dataset provided by the authors. However, analyzing the 4 sites included in Precht et al, but querying the data provided by the authors to FDEP for compliance monitoring, we find that the northern control site (R2NC2) was observed to have a 10% WP prevalence on May 19, 2015. Precht et al. excludes observations from May 2015 in the analysis presented in their study. For the authors’ response to our inquiry about this missing date, see [Response to Rachel Silverstein - Part 2 (WFP & BEG 4.20.18).pdf](Response to Rachel Silverstein - Part 2 (WFP & BEG 4.20.18).pdf)
...
Based on the Precht et al. dataset, only six species are shown to have WP at the four permanent monitoring sites. The tagged coral data do not contain any reports of WP for *Colpophyllia natans*, as reported (Precht et al. Table 1). Photographs provided by FDEP show that the single tagged *C. natans* colony was indeed diseased on Dec. 12, 2014, but data from this date are, for some reason, excluded from the authors’ dataset. The finding is therefore not reproducible from the dataset provided[^8].
...
We find that on July 7, 2015, only 3 out of 55 corals observed (5.5%) were actually recorded in the data as “WP”. If we also include corals that were recorded as having WP *and* corals that were previously recorded as having WP and are now recorded as dead (“WPdead”), the total is 15 out of 55 corals, or 27.3%. Therefore, we are unable to reproduce the result reported in the paper with the provided data. To reach the reported 40% disease prevalence, corals assumed to have died from WP without direct observation must also be counted. Including dead corals in the number of corals described as “showing signs of white plague infection” on a given date overinflates the apparent disease prevalence.

We therefore queried the Precht et al. dataset to determine the highest recorded prevalence of active white plague (“WP” condition code):
...
We find that the dataset provided shows that only 28 out of 69 colonies (40.6%) were recorded as having WP at any point in time. Therefore, the reported disease prevalence of all timepoints cumulatively of 51% must also include corals that were found dead and that authors assumed to have died from white plague.
